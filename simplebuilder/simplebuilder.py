...
def clone_and_build_gbp(repo_url, build_dir, repo_dir):
    ...
    if os.environ['LOCALSUFFIX']:
        run_command(f"cd {repo_name}; dch --rebuild --local {os.environ['LOCALSUFFIX']} 'Add suffix'; \
    ...
    with tempfile.TemporaryDirectory(dir=build_dir) as temp_dir:
        ...
        for item in os.listdir(temp_dir):
            ...
            if os.path.isdir(item_path) and item != filename:
                if os.environ['LOCALSUFFIX']:
                    build_cmd = f"dch --rebuild --local {os.environ['LOCALSUFFIX']} 'Add suffix' && dpkg-buildpackage -uc -us -b"
                else:
                    if rebuild:
