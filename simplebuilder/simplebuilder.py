...
def setup_sbuild_chroot(dist, base_url, extra_repositories, force_chroot=False, chroot_base="/srv/chroot"):
    ...
    if fstab_path.exists():
        ...
        content = re.sub(r'^/sys.*rw,bind.*$', '/sys /sys none rw,rbind 0 0', content, flags=re.MULTILINE)
        content = re.sub(r'^/dev.*rw,bind.*$', '/dev /dev none rw,rbind 0 0', content, flags=re.MULTILINE)
